...
class CourtCountAnalyzer
{
    ...
    public function analyzeCourtCount(string $venueName, ?string $venueAddress = null, ?string $venueWebsite = null): array
    {
        try {
            ...
            $query .= "\n\n⚠️ IMPORTANT EVIDENCE RULES - READ CAREFULLY:";
            $query .= "\nSet evidence_found to TRUE if ANY of these conditions are met:";
            $query .= "\n1. The venue name contains 'Squash Club', 'Squash Centre/Center', 'Squash Facility', or similar - this is STRONG evidence";
            $query .= "\n2. The venue has an active/accessible official website (not Facebook) - this indicates it's operational";
            $query .= "\n3. The website mentions squash in ANY capacity (facilities, bookings, membership, etc.)";
            $query .= "\n4. Google Maps shows the venue as open/active (not permanently closed)";
            $query .= "\n5. Facebook page exists and shows recent activity related to squash";
            $query .= "\n6. ANY online source mentions squash courts at this venue";
            $query .= "\n\nOnly set evidence_found to FALSE if ALL of these are true:";
            $query .= "\n1. The venue name does NOT suggest it's a squash facility";
            $query .= "\n2. No website exists OR website explicitly states venue is closed/demolished";
            $query .= "\n3. Google Maps shows 'PERMANENTLY CLOSED'";
            $query .= "\n4. Multiple sources confirm the venue no longer exists or never had squash courts";
            $query .= "\n\nRemember: We're trying to identify venues that should be DELETED from our database.";
            $query .= "\nBe CONSERVATIVE - when in doubt, set evidence_found to TRUE (it's better to keep a venue than incorrectly delete it).";
            ...
            $response = Http::withHeaders([
                ...
                'input' => $query . "\n\nIMPORTANT: After searching, return ONLY valid JSON in this format: {\"court_count\": <integer or null>, \"confidence\": \"HIGH|MEDIUM|LOW\", \"reasoning\": \"<explanation with source URL>\", \"source_url\": \"<url where info was found>\", \"source_type\": \"VENUE_WEBSITE|SOCIAL_MEDIA|BOOKING_PAGE|GOOGLE_REVIEWS|OTHER\", \"evidence_found\": <boolean>}.\n\nCRITICAL RULES:\n1. PRIORITY: Find the official CLUB/VENUE website first (not estate/development site). If Google Places provided a website, verify it's the correct one:\n   - If it's about residential estate/development and lacks facilities details, it's likely WRONG\n   - Search for the actual club/venue website that has sports facilities information\n   - Look for domains matching the venue name (e.g., victoria.co.za for Victoria Country Club, not vcce.co.za)\n2. ALWAYS check the facilities page (/facilities, /sports, /squash, /amenities) - this is where court counts are most commonly listed.\n3. ⚠️ EVIDENCE FOUND RULES (MOST IMPORTANT):\n   Set evidence_found to TRUE if ANY of these are true:\n   - Venue name contains 'Squash Club/Centre/Center/Facility' (this alone is strong evidence)\n   - Venue has an active website that's accessible (indicates it's operational)\n   - Website mentions squash in ANY way (facilities, bookings, membership, etc.)\n   - Google Maps shows venue as open/active (not permanently closed)\n   - Facebook page exists with squash-related content\n   - ANY online source mentions squash courts at this venue\n   \n   Only set evidence_found to FALSE if ALL of these are true:\n   - Venue name does NOT suggest squash facility\n   - No website OR website says explicitly closed/demolished\n   - Google Maps shows 'PERMANENTLY CLOSED'\n   - Multiple sources confirm venue doesn't exist or never had squash\n   \n   BE CONSERVATIVE: When in doubt, set evidence_found to TRUE. It's better to keep a venue than incorrectly flag it for deletion.\n4. If a Facebook page requires login but you can see it exists, try to find information from other sources (website, Google Maps, reviews).\n5. If the official website mentions squash but doesn't specify count, check Facebook posts, reviews, or other sources for the number.\n6. Be thorough - check multiple sources before concluding no evidence exists.\n7. EXCLUDE squash.players.app from all searches - this is our own directory and should not be used as a source (it would be circular to use our own data to update our own data).\n8. If you find the official website but it doesn't have facilities info on the homepage, search for the facilities page specifically (e.g., victoria.co.za/facilities or mail.victoria.co.za/facilities.htm).\n9. DISTINGUISH between related websites: Some venues have both an estate/development website and a club/venue website. Always use the CLUB website for facilities information.",
                'temperature' => 0.2,
            ]);
